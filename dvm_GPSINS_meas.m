...
%     [R_meas_E_dash, R_meas_e_dash] = eig(R_meas_dash);
    ...
    for i=2:NN
        velYaw(i)   = (gt(3,i)-gt(3,i-1))/dt;
        velPitch(i) = (gt(4,i)-gt(4,i-1))/dt;
        velRoll(i)  = (gt(5,i)-gt(5,i-1))/dt;
    end
...
%       meas = [Tr_total{k}(1,4); Tr_total{k}(3,4);eulAng(2,k)];%; gt_meas(1,k);gt_meas(2,k);gt_meas(3,k)];
      meas = [Tr_ekf_total{k}(1,4); Tr_ekf_total{k}(3,4);eulAngEkf(2,k);-eulAngEkf(3,k);-eulAngEkf(1,k);Tr_ekf_total{k}(2,4); gt_meas(1,k);gt_meas(2,k);gt_meas(3,k);gt(4,k);gt(5,k)];
...
%       meas_store_dash(:,k)= meas_dash; 
      ...
      x_handle=@(x)[...
                x(1) + x(2) * dt + 0.5 * x(14) * (dt ^ 2);  % 1. x
                x(2) + x(14) * dt;  % 2. x vel
                x(3) + x(4) * dt + 0.5 * x(15) * (dt ^ 2);  % 3.z
                x(4) + x(15) * dt;  % 4. z vel
                x(5) + x(6) * dt + 0.5 * x(47) * (dt ^ 2);  % 5. yaw
                x(6) + x(47) * dt;  % 6. yaw rate
                x(7) + x(8) * dt + 0.5 * x(49) * (dt ^ 2);  % 7. roll
                x(8) + x(49) * dt;  % 8. roll rate
                x(9) + x(10) * dt + 0.5 * x(48) * (dt ^ 2);  % 9. pitch
                x(10) + x(48) * dt;  % 10. pitch rate
                x(11) + x(12) * dt + 0.5 * x(16) * (dt ^ 2);  % 11. y
                x(12) + x(16) * dt;  % 12. y vel
                x(13) + x(17)*dt;       % 13. Vxy
                x(14); % (1/m)*((x(18)+x(19))*cos(x(50)) - (x(22)+x(23))*sin(x(50)) + x(20) + x(21) + ms*h*x(47)*x(7)) + x(12)*x(5);      % 14. ax
                x(15); % 15. az
                x(16); % (1/m)*((x(22)+x(23))*cos(x(50)) + (x(18)+x(19))*sin(x(50)) + x(24) + x(25) - ms*h*x(49)) - x(2)*x(5);      % 16. ay
                x(17); % 17. Axy
                x(18); %-Ck_FL * x(30);       % 18. fxFL
                x(19); %-Ck_FR * x(31);       % 19. fxFR
                x(20); %-Ck_RL * x(32);       % 20. fxRL
                x(21); %-Ck_RR * x(33);       % 21. fxRR
                x(22); %-Ca_FL * x(43);       % 22. fyFL
                x(23); %-Ca_FR * x(44);       % 23. fyFR
                -Ca_RL * x(45);       % 24. fyRL
                -Ca_RR * x(46);       % 25. fyRR
                %ks_FL*(-x(54)) + c_FL*(-x(58)) + m_wFL*g + 
                (0.5*m*g + ((m*h*x(16))/tf))*(b/(a+b)) - 0.5*m*x(14)*(h/(a+b));       % 26. fzFL
                %ks_FR*(-x(55)) + c_FR*(-x(59)) + m_wFR*g + 
                (0.5*m*g - ((m*h*x(16))/tf))*(b/(a+b)) - 0.5*m*x(14)*(h/(a+b));       % 27. fzFR
                %ks_RL*(-x(56)) + c_RL*(-x(60)) + m_wRL*g + 
                (0.5*m*g + ((m*h*x(16))/tr))*(a/(a+b)) + 0.5*m*x(14)*(h/(a+b));       % 28. fzRL
                %ks_RR*(-x(57)) + c_RR*(-x(61)) + m_wRR*g + 
                (0.5*m*g - ((m*h*x(16))/tr))*(a/(a+b)) + 0.5*m*x(14)*(h/(a+b));       % 29. fzRR
                x(30); %x(34)*wheelr/x(38) - 1;      % 30. slip ratio FL
                x(31); %x(35)*wheelr/x(39) - 1;      % 31. slip ratio FR
                x(32); %x(36)*wheelr/x(40) - 1;      % 32. slip ratio RL
                x(33); %x(37)*wheelr/x(41) - 1;      % 33. slip ratio RR
                x(34);       % 34. wheel angular velocity FL
                x(35);       % 35. FR
                x(36);       % 36. RL
                x(37);       % 37. RR
                sqrt(x(2)^2 + x(12)^2) + x(6)*((tf/2)-a*x(42));    % 38. FL acutal wheel velocity
                sqrt(x(2)^2 + x(12)^2) + x(6)*((-tf/2)-a*x(42));   % 39. FR
                sqrt(x(2)^2 + x(12)^2) + x(6)*((tf/2)+b*x(42));    % 40. RL
                sqrt(x(2)^2 + x(12)^2) + x(6)*((-tf/2)+b*x(42));   % 41. RR
                atan(x(12)/x(2));        % 42. Beta
                x(43); % 43. x(50) - atan((x(12) + b*x(6))/(x(2) + tf*x(6)*0.5)); % alpha FL
                x(44); % 44. x(50) - atan((x(12) + b*x(6))/(x(2) - tf*x(6)*0.5)); % alpha FR
                atan((-x(12) + b*x(6))/(x(2) + tr*x(6)*0.5));        % 45. alpha RL
                atan((-x(12) + b*x(6))/(x(2) - tr*x(6)*0.5));        % 46. alpha RR
                x(47); %(1/Iz)*(tf*0.5*(x(18)*cos(x(50)) - x(22)*sin(x(50)) - x(19)*cos(x(50)) + x(23)*sin(x(50))) + tr*0.5*(x(20)-x(21)) + a*(x(22)*cos(x(50))+x(18)*sin(x(50))+x(23)*cos(x(50))+x(19)*sin(x(50))) - b*(x(24)-x(25)));      % 47. yawA
                (1/Iy)*(b*(x(29)+x(28)) - a*(x(27)+x(26)));  % 48. pitchA
                x(49); %ms*g*h*x(7) - ms*h*(x(16) + x(14)*x(8)) + (x(26)+x(29)-x(28)-x(27))*d;       % 49. rollA
                x(50);       % 50. delta
                x(51) + x(52)*dt + 0.5*x(53)*(dt^2);       % 51. zs sprung mass displacement
                x(52) + x(52)*dt;       % 52. zsR sprung mass velocity
                (1/ms)*(x(26) + x(27) + x(28) + x(29));       % 53. zsA sprung mass accelration
                x(51) - a*x(9) + tf*x(7)*0.5;    % 54. zs_FL
                x(51) - a*x(9) - tf*x(7)*0.5;    % 55. zs_FR
                x(51) + b*x(9) - tf*x(7)*0.5;    % 56. zs_RL
                x(51) + b*x(9) + tf*x(7)*0.5;    % 57. zs_RR
                x(52) - a*x(10) + tf*0.5*(1/x(8));    % 58. zsR_FL
                x(52) - a*x(10) - tf*0.5*(1/x(8));    % 59. zsR_FR
                x(52) + b*x(10) - tf*0.5*(1/x(8));    % 60. zsR_RL
                x(52) + b*x(10) + tf*0.5*(1/x(8));    % 61. zsR_RR
          ];
...
%       y_handle=@(x)[x(2)/dt;x(4)/dt;x(6)/dt];      % measurement equation
      y_handle=@(x)[x(1)+d(1);x(3)+d(2);x(5)+d(3);x(9)+d(4);x(7)+d(5);x(11)+d(6);x(1)+d(7);x(3)+d(8);x(5)+d(9);x(9)+d(10);x(7)+d(11)];
...
%   hold off;
  ...
  subplot(5,2,8)
  plot(1:NN, velYaw, 'b-', 1:NN, x_store(6,:),'r--');
  subplot(5,2,9)
  plot(1:NN, velPitch, 'b-', 1:NN, x_store(10,:),'r--');
  subplot(5,2,10)
  plot(1:NN, velRoll, 'b-', 1:NN, x_store(8,:),'r--');
